...
const customFetch = async (url, { body, ...customConfig }) => {
  // get token from localstorage // which is useful when we login or logout
  const token = window.localStorage.getItem(LOCALSTORAGE_TOKEN_KEY);

  const headers = {
    'content-type': 'application/x-www-form-urlencoded',
  };
  // if token is exit then set in headers authorization as a bearer key
  if (token) {
  ...
  // console.log('we are just checking:', config);
  ...
  try {
    ...
    if (data.success) {
      return {
        data: data.data, //data have user details like user name ,user emailId, userId,tokenId
        success: true,
      };
    }
